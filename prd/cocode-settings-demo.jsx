...
export default function Settings() {
  ...
  const [activeNav, setActiveNav] = useState("providers");
  ...
  return (
    <div style={{ fontFamily: "'Inter', -apple-system, sans-serif", background: t.bg, color: t.text, height: "100vh", display: "flex", overflow: "hidden", fontSize: 13 }}>
      ...
      <div style={{ width: 224, background: t.sidebar, borderRight: `1px solid ${t.border}`, display: "flex", flexDirection: "column", flexShrink: 0 }}>
        ...
        <div style={{ flex: 1, padding: "10px 8px", display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
          <button style={{ width: "100%", textAlign: "left", padding: "8px 10px", borderRadius: 8, background: t.card, border: `1px solid ${t.border}`, cursor: "pointer", fontFamily: "inherit", color: t.text, fontSize: 13, fontWeight: 600, display: "flex", alignItems: "center", gap: 8, marginBottom: 2 }}>
            <span>⚙</span> 设置
          </button>
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        <div style={{ width: 200, background: t.surface, borderRight: `1px solid ${t.border}`, padding: "20px 10px", flexShrink: 0 }}>
          <div style={{ fontSize: 10, color: t.textMuted, padding: "0 8px 8px", textTransform: "uppercase", letterSpacing: 0.8, fontWeight: 600 }}>设置</div>
          {NAV.map(item => (
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: p.connected ? 12 : 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        ...
                        <span style={{
                          ...
                          background: p.connected ? t.greenBg : t.accentSoft,
                          color: p.connected ? t.green : t.textSub,
                          border: `1px solid ${p.connected ? t.greenBorder : t.border}`,
                        }}>{p.connected ? "已接入" : "未接入"}</span>
                      </div>
                      <button style={{
                        fontSize: 12, padding: "5px 14px", borderRadius: 7, cursor: "pointer", fontFamily: "inherit", fontWeight: 500,
                        border: `1px solid ${p.connected ? t.border : t.accent}`,
                        background: p.connected ? "none" : t.accent,
                        color: p.connected ? t.textSub : t.accentText,
                      }}>
                        {p.connected ? "编辑" : "接入"}
                      </button>
                    </div>
                    {p.connected && (
                      <>
                        ...
                        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                          {p.models.map(m => (
                            <span key={m} style={{ fontSize: 11, color: t.textSub, background: t.accentSoft, padding: "2px 9px", borderRadius: 20, border: `1px solid ${t.border}` }}>{m}</span>
                          ))}
                        </div>
                      </>
                    )}
          ...
          {activeNav === "agents" && (
            <div>
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 24 }}>
                ...
                <button style={{ fontSize: 12, padding: "6px 16px", borderRadius: 8, border: "none", background: t.accent, color: t.accentText, cursor: "pointer", fontFamily: "inherit", fontWeight: 600, flexShrink: 0 }}>
                  + 新建 Agent
                </button>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {AGENTS.map(agent => (
                  <div key={agent.id} style={{ background: t.card, border: `1px solid ${t.border}`, borderRadius: 10, overflow: "hidden" }}>
                    <button onClick={() => setExpandedAgent(expandedAgent === agent.id ? null : agent.id)} style={{
                      width: "100%", textAlign: "left", padding: "14px 18px", background: "none", border: "none", cursor: "pointer", fontFamily: "inherit",
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        ...
                        <span style={{
                          fontSize: 10, padding: "2px 8px", borderRadius: 20, fontWeight: 600,
                          background: agent.type === "builtin" ? t.accentSoft : t.amberBg,
                          color: agent.type === "builtin" ? t.textSub : t.amber,
                          border: `1px solid ${agent.type === "builtin" ? t.border : t.amberBorder}`,
                        }}>{agent.type === "builtin" ? "内置" : "自定义"}</span>
                      </div>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <div style={{ padding: "0 18px 16px", borderTop: `1px solid ${t.borderSoft}` }}>
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 14 }}>
                          <div>
                            ...
                            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                              {agent.skills.map(s => (
                                <span key={s} style={{ fontSize: 11, color: t.textSub, background: t.accentSoft, padding: "2px 9px", borderRadius: 20, border: `1px solid ${t.border}` }}>{s}</span>
                              ))}
                            </div>
                          </div>
                          <div>
                            ...
                            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                              {agent.tools.map(tool => (
                                <span key={tool} style={{ fontSize: 11, color: t.textSub, background: t.accentSoft, padding: "2px 9px", borderRadius: 20, border: `1px solid ${t.border}` }}>{tool}</span>
                              ))}
                            </div>
                          </div>
                        </div>
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 6, marginBottom: 18 }}>
                {SKILL_CATEGORIES.map(cat => (
                  <button key={cat} onClick={() => setSkillCat(cat)} style={{
                    fontSize: 12, padding: "5px 14px", borderRadius: 20, cursor: "pointer", fontFamily: "inherit",
                    border: `1px solid ${skillCat === cat ? t.accent : t.border}`,
                    background: skillCat === cat ? t.accent : "none",
                    color: skillCat === cat ? t.accentText : t.textSub,
                    fontWeight: skillCat === cat ? 600 : 400,
                  }}>
                    {cat}
                  </button>
                ))}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                {filteredSkills.map(skill => (
                  <div key={skill.id} style={{ background: t.card, border: `1px solid ${t.border}`, borderRadius: 10, padding: "14px 16px", display: "flex", flexDirection: "column", gap: 8 }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      <span style={{ fontWeight: 600, fontSize: 13, color: t.text }}>{skill.name}</span>
                      <div style={{ display: "flex", gap: 6 }}>
                        <span style={{
                          fontSize: 10, padding: "2px 7px", borderRadius: 20, fontWeight: 600,
                          background: skill.builtin ? t.accentSoft : t.amberBg,
                          color: skill.builtin ? t.textSub : t.amber,
                          border: `1px solid ${skill.builtin ? t.border : t.amberBorder}`,
                        }}>{skill.builtin ? "内置" : "自定义"}</span>
                    ...
                    <div style={{ fontSize: 12, color: t.textSub, lineHeight: 1.6 }}>{skill.desc}</div>
                    {!skill.builtin && (
          ...
          {activeNav === "appearance" && (
            <div>
              ...
              <div style={{ display: "flex", gap: 12 }}>
                {[{ key: true, label: "暗色", bg: "#111", text: "#fff", card: "#1a1a1a", border: "#222" }, { key: false, label: "亮色", bg: "#f5f5f3", text: "#111", card: "#fff", border: "#e8e8e4" }].map(theme => (
                  <button key={String(theme.key)} onClick={() => setDark(theme.key)} style={{
                    padding: "16px", borderRadius: 12, cursor: "pointer", fontFamily: "inherit",
                    border: `2px solid ${dark === theme.key ? t.accent : t.border}`,
                    background: t.card, width: 140,
                  }}>
                    <div style={{ width: "100%", height: 80, borderRadius: 8, background: theme.bg, border: `1px solid ${theme.border}`, marginBottom: 10, overflow: "hidden", position: "relative" }}>
                      ...
                      <div style={{ position: "absolute", left: 44, top: 10, width: 60, height: 6, borderRadius: 3, background: theme.text, opacity: 0.8 }} />
                      <div style={{ position: "absolute", left: 44, top: 22, width: 45, height: 4, borderRadius: 2, background: theme.text, opacity: 0.3 }} />
                    </div>
          ...
          {activeNav === "account" && (
            <div>
              ...
              <div style={{ background: t.card, border: `1px solid ${t.border}`, borderRadius: 10, padding: "20px" }}>
                ...
                <button style={{ fontSize: 12, padding: "6px 16px", borderRadius: 8, border: `1px solid ${t.border}`, background: "none", color: t.textSub, cursor: "pointer", fontFamily: "inherit" }}>
                  编辑资料
                </button>
              </div>
            </div>
              ...
              <div style={{ background: t.card, border: `1px solid ${t.border}`, borderRadius: 10, padding: "24px", display: "flex", flexDirection: "column", gap: 14 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ width: 40, height: 40, background: t.accent, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <span style={{ color: t.accentText, fontSize: 14, fontWeight: 900, letterSpacing: -1 }}>cc</span>
                  </div>
                  <div>
